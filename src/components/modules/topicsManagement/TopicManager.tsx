...
import { deleteTopic, Topic } from "@/service/topics/topic.service";
...
export function TopicManager({ initialTopics, categories }: TopicManagerProps) {
  ...
  }, [topics, query, statusFilter, categoryFilter]);
  ...
  function handleSaved() {
    setFormOpen(false);
    showToast(editing ? "টপিক আপডেট হয়েছে" : "টপিক তৈরি হয়েছে");
  ...
  function confirmDelete() {
    ...
    startDeleteTransition(async () => {
      const result = await deleteTopic(target.id);
      if (result.success) {
        setTopics((prev) => prev.filter((t) => t.id !== target.id));
        showToast(result.message ?? "টপিক মুছে ফেলা হয়েছে");
      } else {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="relative w-full lg:max-w-sm">
          ...
          <input
            ...
            placeholder="টপিক বা ক্যাটাগরি খুঁজুন…"
            className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-800 shadow-sm outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-700 shadow-sm outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20"
          >
          ...
          <button
            onClick={openCreate}
            className="inline-flex items-center gap-2 rounded-xl bg-cyan-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2"
          >
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
                ...
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-slate-500">
                      {topics.length === 0
                        ? "এখনও কোনো টপিক যোগ করা হয়নি — শুরু করতে “নতুন টপিক” চাপুন।"
            ...
            {filtered.map((topic) => (
              <tr key={topic.id} className="group transition hover:bg-cyan-50/40">
                ...
                <td className="px-6 py-4">
                  <StatusPill active={topic.isActive} />
                </td>
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-1 opacity-70 transition group-hover:opacity-100">
                    <button
                      onClick={() => openEdit(topic)}
                      className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-cyan-600 transition hover:bg-cyan-50"
                    >
      ...
      <ConfirmDialog
        ...
        description={`"${pendingDelete?.name ?? ""}" মুছে ফেলা হলে এর সাথে যুক্ত প্রশ্নের উপর প্রভাব পড়তে পারে। প্রশ্ন থাকলে এটি শুধু নিষ্ক্রিয় করা হবে।`}
        pending={isDeleting}
