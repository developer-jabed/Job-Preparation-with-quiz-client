...
import { deleteSubject, Subject } from "@/service/subject/subject.service";
import { StatusPill } from "./StatusPill";
import { SubjectFormDialog } from "./SubjectDialogue";
...
export function SubjectManager({ initialSubjects }: SubjectManagerProps) {
  ...
  function handleSaved() {
    setFormOpen(false);
    showToast(editing ? "বিষয় আপডেট হয়েছে" : "বিষয় তৈরি হয়েছে");
  ...
  function confirmDelete() {
    ...
    startDeleteTransition(async () => {
      const result = await deleteSubject(target.id);
      if (result.success) {
        setSubjects((prev) => prev.filter((s) => s.id !== target.id));
        showToast(result.message ?? "বিষয় মুছে ফেলা হয়েছে");
      } else {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-800 shadow-sm outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
          />
        </div>
                ...
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-slate-500">
                      {subjects.length === 0
                        ? "এখনও কোনো বিষয় যোগ করা হয়নি — শুরু করতে “নতুন বিষয়” চাপুন।"
            ...
            {filtered.map((subject) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-slate-600">
                  {subject._count?.categories ?? 0}
                </td>
                <td className="px-6 py-4 text-slate-600">
                  {subject._count?.questions ?? 0}
                </td>
                <td className="px-6 py-4">
                  <StatusPill active={subject.isActive} />
                </td>
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-1 opacity-70 transition group-hover:opacity-100">
                    <button
                      onClick={() => openEdit(subject)}
                      className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-indigo-600 transition hover:bg-indigo-50"
                    >
      ...
      <ConfirmDialog
        ...
        description={`"${pendingDelete?.name ?? ""}" মুছে ফেলা হলে এটির সাথে যুক্ত ক্যাটাগরি ও প্রশ্নের উপর প্রভাব পড়তে পারে। প্রশ্ন থাকলে এটি শুধু নিষ্ক্রিয় করা হবে, সম্পূর্ণ মুছে যাবে না।`}
        pending={isDeleting}
